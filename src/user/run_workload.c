...
__attribute__((always_inline)) static inline long sys_call(long n, long a0, long a1, long a2, long a3, long a4, long a5) {
...
static inline long sys_getpid(void) { return sys_call(SYSCALL_GETPID, 0, 0, 0, 0, 0, 0); }
static inline long sys_yield(void) { return sys_call(SYSCALL_YIELD, 0, 0, 0, 0, 0, 0); }
static inline long sys_ticks(void) { return sys_call(SYSCALL_TICKS, 0, 0, 0, 0, 0, 0); }
...
static void uput_u64_line(const char *prefix, uint64_t v) {
    char b[64];
    int p = 0;
    while (prefix[p]) { b[p] = prefix[p]; p++; }
    append_u64(b, &p, v);
    b[p++] = '\n';
    sys_write(1, b, p);
}
...
int main() {
    ...
    long got = sys_get_workload(g_workload, sizeof(g_workload));
    if (got <= 0) {
    ...
    if (streq(workload_name(), "smoke")) {
        ...
        sleep_t = 0;
        exit_code = 1;
    } else if (streq(workload_name(), "syscall_bench")) {
        sleep_t = 0;
        uputs("BENCH_BEGIN\n");
        ...
        for (long i = 0; i < iters; i++) {
            ...
            if ((i % 1000) == 0) {
                uput_u64_line("BENCH_I=", (uint64_t)i);
            }
